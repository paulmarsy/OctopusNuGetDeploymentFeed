...
$newApplicationPackagePath = Join-Path $Env:TEMP 'OctopusDeployNuGetFeed.ServiceFabric.pkg'
if (Test-Path $newApplicationPackagePath) {
	Remove-Item $newApplicationPackagePath -Recurse -Force
}
Copy-Item $ApplicationPackagePath $newApplicationPackagePath -Recurse -Force
$ApplicationPackagePath = $newApplicationPackagePath
